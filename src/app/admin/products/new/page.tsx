...
export default function NewProductPage() {
  ...
  const [dragOver, setDragOver] = useState<'video' | 'content' | 'icon' | null>(null);
  ...
  const handleDrop = (e: React.DragEvent, type: 'video' | 'content' | 'icon') => {
    e.preventDefault();
    setDragOver(null);
  ...
  const handleDragEnter = (e: React.DragEvent, type: 'video' | 'content' | 'icon') => {
    e.preventDefault();
    setDragOver(type);
  };
  ...
  const handleSave = async (isPublic: boolean) => {
    ...
    try {
      ...
      const result = await response.json();
      if (result.success) {
        alert(isPublic ? 'プロダクトを公開しました！' : 'プロダクトを非公開として保存しました。');
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">新規プロダクト投稿</h1>
            <p className="text-gray-600 mt-2">新しいプロダクトを作成します</p>
          </div>
        </div>
